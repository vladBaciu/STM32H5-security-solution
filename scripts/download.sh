...
if [ -z "$stm32programmercli" ] || [ -z "$projectdir" ]; then
  if [ -e "$thispath/env.sh" ]; then
    source "$thispath/env.sh"
    if [ -e "$provisioning_dir/env.sh" ]; then
...
path_binary="$thispath/../Binary/appli_enc_sign.hex"
xml="$thispath/../Images/SM_Code_Image.xml"
read_xml="$provisioning_dir/SM/Helper/read_xml.sh"
