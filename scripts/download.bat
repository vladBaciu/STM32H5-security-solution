...
set path_binary="%projectdir%\..\Binary\appli_enc_sign.hex"
...
for /f "delims=" %%a IN ('call "%provisioning_dir%/SM/Helper/read_xml.bat" "%projectdir%/Images/SM_Code_Image.xml" "Firmware area Size"') do (set size_slot=%%a)
for /f "delims=" %%a IN ('call "%provisioning_dir%/SM/Helper/read_xml.bat" "%projectdir%/Images/SM_Code_Image.xml" "Firmware download area offset"') do (set offset=%%a)
