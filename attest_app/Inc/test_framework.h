...
struct test_result_t
{
  ...
  const char *info_msg;    /*!< Information message to show in case of
                              *   failure
                              */
  const char *filename;    /*!< Filename where the failure has occurred */
  uint32_t    line;        /*!< Line where the failure has occurred */
};
...
/**
  ...
  * \param[in]  filename  Filename where the error has occurred
  * \param[in]  line      Line in the file where the error has occurred
...
#define TEST_FAIL(info_msg) (void)printf("\nTest FAILED : %s\n",info_msg)
#define TEST_STATUS(status) (void)printf("\nStatus : %d \n", (int)status)
