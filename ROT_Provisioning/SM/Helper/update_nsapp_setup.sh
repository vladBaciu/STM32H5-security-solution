...
if [ $todate -eq 0 ]; then
  ...
  case $compiler in
    ...
    runAppliCfg linker -xml "$xml_config_gen" -nxml "$code_size" -n CODE_SIZE "$linkerfile" --vb
    if [ $? -ne 0 ]; then error_config 'cube1'; fi
    runAppliCfg linker -xml "$xml_config_gen" -nxml "$code_offset" -n CODE_OFFSET "$linkerfile" --vb
    if [ $? -ne 0 ]; then error_config 'cube2'; fi
    runAppliCfg linker -xml "$xml_config_gen" -nxml "$ram_s_ns_if_size" -n RAM_S_NS_IF_SIZE "$linkerfile" --vb
    if [ $? -ne 0 ]; then error_config 'cube3'; fi
    runAppliCfg linker -xml "$xml_config_gen" -nxml "$secure_sram_end_address" -n RAM_S_END -e "val1-0x10000000" "$linkerfile" --vb
    if [ $? -ne 0 ]; then error_config 'cube4'; fi
    runAppliCfg linker -xml "$xml_config_gen" -nxml "$ns_reserved_area_offset" -n RESERVED_AREA_OFFSET "$linkerfile" --vb
    if [ $? -ne 0 ]; then error_config 'cube5'; fi
    runAppliCfg linker -xml "$xml_config_gen" -nxml "$ns_reserved_area_size" -n RESERVED_AREA_SIZE "$linkerfile" --vb
  ...
  if [ ! -e "$code_image_file_project" ] || [ ! -e "$code_image_bin_file_project" ]; then
     xml_copied=2
  ...
  runAppliCfg xmlval -xml "$xml_config_gen" -nxml "$code_size" -n "$fw_size" -e "val1*$SECTOR_SIZE" "$code_image_file_project" --vb
  if [ $? -ne 0 ]; then error_config 'img1'; fi
  runAppliCfg xmlval -xml "$xml_config_gen" -nxml "$code_offset" -n "$rom_fixed" -e "(val1*$SECTOR_SIZE)+0x8000000" "$code_image_file_project" --vb
  if [ $? -ne 0 ]; then error_config 'img1a'; fi
  runAppliCfg xmlval -xml "$xml_config_gen" -nxml "$code_dwl_offset" -n "$fw_dwl_offset" -e "(val1*$SECTOR_SIZE)+0x8000000" "$code_image_file_project" --vb
  if [ $? -ne 0 ]; then error_config 'img1b'; fi

  runAppliCfg xmlval -xml "$xml_config_gen" -nxml "$code_size" -n "$fw_size" -e "val1*$SECTOR_SIZE" "$code_image_bin_file_project" --vb
  if [ $? -ne 0 ]; then error_config 'img2'; fi
  runAppliCfg xmlval -xml "$xml_config_gen" -nxml "$code_offset" -n "$rom_fixed" -e "(val1*$SECTOR_SIZE)+0x8000000" "$code_image_bin_file_project" --vb
  if [ $? -ne 0 ]; then error_config 'img2a'; fi

fi
success_config "$xml_copied"
